...
export default function Home(): any {
  ...
  const {
    ...
    currentVideos: state.app.currentVideos,
    favoriteVideos: state.app.favoriteVideos,
  ...
  const onInputValue = (payload: string) => {
  ...
  const onGetVideos = () => {
    ...
    if (inputValue) {
      api
        .get(`https://api.twitch.tv/helix/search/channels?query=${inputValue}`)
        .then(({ data: { data } }) => {
          api
            .get(
              `https://api.twitch.tv/helix/videos?user_id=${
                data.filter(
                  (item) =>
                    item.broadcaster_login.toLowerCase() ===
            ...
            .then(({ data: { data } }) =>
              dispatch(
                setCurrentVideos(
                  data.map((item) =>
                    item ? { ...item, favoriteVideo: false } : null
                  )
                )
              )
            );
        ...
        .catch(() =>
          alert(
            "Не удалось получить информацию с сервера, возможно, пользователя с данным именем не существует или у него нет сохраненных видео "
          )
        )
        .finally(() => onSetLoading());
      onInputValue("");
    } else {
    ...
    <div className="container">
      ...
      <div className="main">
        <div className="input">
          ...
          <input
            value={inputValue}
            onChange={(e: any) => onInputValue(e.target.value)}
            className="field"
            type="text"
            placeholder="Название канала"
          />
          <button onClick={onGetVideos} className="button" type="submit">
            Найти
          </button>
        </div>
            ...
            {!loading ? (
              currentVideos && currentVideos.length ? (
                    ...
                    <a target="_blank" href={video.url} rel="noreferrer">
                      ...
                      <i
                        className={classNames({
                          active: favoriteVideos
                            .map((item) => item.id)
                            .includes(video.id),
                        })}
                        onClick={(event) =>
                          addFavoriteVideo(
                            { ...video, favoriteVideo: true },
                            event
                          )
                        }
                      >
                ...
                <h3 className="content__title">
                  У данного пользователя нет видео
                </h3>
              )
            ) : (
              <h3 className="content__title">Идёт загрузка...</h3>
            )}
          </div>
        </div>
      </div>
